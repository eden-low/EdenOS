...
describe('UID scoped settings repository', () => {
  it('reads a persisted settings document and ignores pending local snapshots', () => {
    ...
    expect(next).toHaveBeenCalledWith({ bodyWeightKg: null, heightCm: null, monthlyBudgetSen: null, savingsGoalSen: null })
    callback({ metadata: { fromCache: false, hasPendingWrites: false }, exists: () => true,
      data: () => ({ bodyWeightKg: 71.5, heightCm: 175, monthlyBudgetSen: 20025, savingsGoalSen: 150050 }) })
    expect(next).toHaveBeenLastCalledWith({ bodyWeightKg: 71.5, heightCm: 175, monthlyBudgetSen: 20025, savingsGoalSen: 150050 })
  })
  ...
  it('writes valid weight and money only to the owner document', async () => {
    ...
    await repository.saveHeight(175)
    await repository.saveMonthlyBudget(20025)
    await repository.saveSavingsGoal(150050)
    expect(mock.setDoc).toHaveBeenCalledTimes(5)
    expect(mock.setDoc.mock.calls[2][1]).toEqual({ heightCm: 175, updatedAt: { server: true } })
    expect(mock.setDoc.mock.calls[3][0]).toEqual(['users', 'owner', 'settings', 'preferences'])
    expect(mock.setDoc.mock.calls[3][1]).toEqual({ monthlyBudgetSen: 20025, updatedAt: { server: true } })
    expect(mock.setDoc.mock.calls[4][1]).toEqual({ savingsGoalSen: 150050, updatedAt: { server: true } })
    expect(mock.setDoc.mock.calls[4][2]).toEqual({ merge: true })
    await expect(repository.saveBodyWeight(0)).rejects.toThrow()
    await expect(repository.saveHeight(175.5)).rejects.toThrow()
