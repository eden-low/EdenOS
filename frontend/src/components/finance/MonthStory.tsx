...
export function MonthStory({ summary }: { summary: FinanceSummary }) {
  return <section className="dashboard-card mt-4 p-5 sm:p-6" aria-labelledby="month-story-heading">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Story label="Savings rate" value={summary.savingsRatePercent === null ? 'Needs income data' : `${Math.round(summary.savingsRatePercent)}%`} detail={<><FinancialAmount amountSen={summary.netCashflowSen} /> net cashflow</>} />
      ...
      <Story label="Largest category" value={summary.largestCategory?.label ?? 'No spending yet'} detail={summary.largestCategory ? <FinancialAmount amountSen={summary.largestCategory.spentSen} /> : 'Add an expense to begin'} />
