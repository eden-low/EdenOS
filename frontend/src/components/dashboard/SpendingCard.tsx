...
export function SpendingCard({ spending, finance, status, incomeStatus, error, incomeError, onRetry, onRetryIncome }: SpendingCardProps) {
  const { settings, status: settingsStatus } = useUserSettings()
  const loaded = status === 'loaded' && incomeStatus === 'loaded'
  const remainingSen = settings.monthlyBudgetSen === null ? null : Math.max(0, settings.monthlyBudgetSen - finance.expenseSen)
  return <section aria-label={`${spending.month} finance overview`} className="dashboard-card order-1 col-span-2 bg-[var(--surface-secondary)] p-5 sm:p-6 md:col-span-6 xl:col-span-5">
    ...
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border-subtle)] pt-4"><p className="text-xs text-[var(--text-muted)]">Budget tracks expenses only. Net cashflow is not savings.</p>{settingsStatus === 'loaded' && <MoneySettingsDialog kind="budget" />}</div>
  </section>
}
