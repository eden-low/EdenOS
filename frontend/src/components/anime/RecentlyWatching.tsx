...
export function RecentlyWatching({ onOpen }: { onOpen: (target: AnimePlayerTarget) => void }) {
  ...
  return <section className="mt-8" aria-labelledby="recent-anime-heading"><h2 id="recent-anime-heading" className="section-label">{t('recentlyWatching')}</h2><div className="mt-3 flex gap-3 overflow-x-auto pb-2">{recent.map((item) => <article key={item.externalId} className="flex min-w-[16rem] items-center gap-3 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-primary)] p-3">{item.coverUrl && <img src={item.coverUrl} alt="" className="h-20 w-14 rounded-lg object-cover" />}<div className="min-w-0"><h3 className="truncate text-sm font-semibold">{item.title}</h3><p className="mt-1 text-xs text-[var(--text-secondary)]">{t('episode')} {item.currentEpisode}</p><Button type="button" variant="ghost" className="mt-1 min-h-8 px-0 text-xs" onClick={() => onOpen({ externalId: item.externalId, title: item.title, coverUrl: item.coverUrl, totalEpisodes: item.totalEpisodes })}>{t('continue')}</Button></div></article>)}</div></section>
}
