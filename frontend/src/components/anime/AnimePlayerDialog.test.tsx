...
vi.mock('./AnimeVideoPlayer', () => ({ AnimeVideoPlayer: ({ resumeAt, onProgress, onPause, onPrevious, onNext }: { resumeAt: number; onProgress: (p: number, d: number) => void; onPause: (p: number, d: number) => void; onPrevious: () => void; onNext: () => void }) => <div data-testid="video" data-resume={resumeAt}><button onClick={() => onProgress(90, 100)}>progress</button><button onClick={() => onPause(50, 100)}>pause</button><button onClick={onPrevious}>video previous</button><button onClick={onNext}>video next</button></div> }))
