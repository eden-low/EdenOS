...
export function ExpensesPage({ onOpenRecords }: { onOpenRecords: () => void }) {
  ...
  return <Page>
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Finance summary">
      ...
      <article className="finance-summary-card finance-summary-card-amber"><div className="flex items-start justify-between gap-3"><p className="section-label">Goal Allocations</p><span className="finance-summary-icon"><Target size={19} /></span></div><FinancialAmount amountSen={summary.goalAllocationsSen} className="metric-value mt-4 text-2xl font-semibold" /><p className="mt-2 text-xs text-[var(--text-muted)]">Reserved this month · not an expense</p></article>
    </section>
