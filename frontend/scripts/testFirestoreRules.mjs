...
try {
  ...
  await assertSucceeds(setDoc(own, { heightCm: 175, updatedAt: serverTimestamp() }, { merge: true }))
  ...
  assert.equal(saved?.heightCm, 175)
  ...
  await assertFails(setDoc(own, { heightCm: 175.5, updatedAt: serverTimestamp() }, { merge: true }))
  await assertFails(setDoc(own, { heightCm: 251, updatedAt: serverTimestamp() }, { merge: true }))
